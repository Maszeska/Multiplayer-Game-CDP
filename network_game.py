...
def main():
    ...
    game_state = "shaking"  # Single-player life cycle state
    player_state = "shaking"
    remote_player_state = "shaking"
    active_bombs = []
    remote_bombs = []
    ...
    while running:
        ...
        if game_state == "playing" and player_state == "playing":
        ...
        if game_state == "playing":
            # Keep sending local position for peer prediction/sync.
        ...
        if game_state == "menu":
            ...
            for bomb in active_bombs:
                ...
                if bomb.state == "exploding":
                    ...
                    if player_pos in bomb.blast_tiles:
                        player.take_damage()
                    remote_pos = remote_player.get_grid_pos(game_board)
                    if remote_pos in bomb.blast_tiles:
                        remote_player.take_damage()
            ...
            if game_state == "playing":
                ...
                remote_player.update_timers()

            if game_state == "shaking":
                if player.shake_timer >= SHAKE_DURATION:
                    game_state = "hatching"
                    player_state = "hatching"
                    remote_player_state = "hatching"
                ...
                if player.lives <= 0 and player_state == "playing":
                    player_state = "dying"
                    player.death_frame_index = 0
                if remote_player.lives <= 0 and remote_player_state == "playing":
                    remote_player_state = "dying"
                    remote_player.death_frame_index = 0
                if player_state == "dying" and int(player.death_frame_index) >= len(player.death_frames) - 1:
                    game_state = "game_over"
                if remote_player_state == "dying" and int(remote_player.death_frame_index) >= len(remote_player.death_frames) - 1:
                    game_state = "game_over"

            player.draw(screen, player_state, is_moving)
            remote_player.draw(screen, remote_player_state, False)
